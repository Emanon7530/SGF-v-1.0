Public Class EjecutarCierre
    ...
    Private Sub MarcarCierre(ByVal pFecha As DateTime)
        ...
        SelectStmt = "select * from cgperdd where perdr_year = " & pFecha.Year.ToString & " and perdr_num = " & pFecha.Month.ToString
        ...
        SelectStmt = "select sum(onhand * cost) from ivcierrm where cierr_date = '" & pFecha.ToString("yyyy/MM/dd") & "'"
        ...
        ''''FechaAnt = pFecha.AddMonths(-1)
        ''''SelectStmt = _
        ''''                        "SELECT SUM(BALANCE) AS Balance FROM (SELECT " & _
        ''''                        "    cgacctd.balance " & _
        ''''                        "        FROM " & _
        ''''                        "    cgacctm cgacctm INNER JOIN cgacctd cgacctd " & _
        ''''                        "   ON cgacctm.acct_no = cgacctd.acct_no " & _
