...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.LibXConnector1.Sources.AddRange(New LibX.LibXDbSourceTable() {Me.LibXDbSourceTable5, Me.LibXDbSourceTable6})
        ...
        Me.Text = "Documentos Conciliación"
...
#End Region
    ...
    Private Sub LibXConnector1_SettingDefaultNewValues(ByVal sender As Object, ByVal e As LibX.SettingDefaultNewValues) Handles LibXConnector1.SettingDefaultNewValues
        Try
            ...
            BuscarPeriodo(LibX.Data.Manager.Connection.GetDate)
    ...
    Private Function BalancearEntrada() As Decimal
        ...
        Try
            ...
            '    If MessageBox.Show("Documento No Balanceado, desea guardarlo?", "Documento No Balancedo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            '        Diferencia = 0
            '    End If
            'End If
    ...
    Private Sub LibXConnector1_ExecutingAction(ByVal sender As Object, ByVal e As LibX.ExecutingActionEventArgs) Handles LibXConnector1.ExecutingAction
        ...
        Try
            ...
            AndAlso Not LibX.IsNull(LibXConnector1.CurrentDataRow!doc_origen) _
            ...
            If LibXConnector1.HasRecords = True And LibXConnector1.IsDataEditing = True Then
                '''If e.Action = LibX.LibxConnectionActions.Edit And _
                '''   Not LibX.IsNull(LibXConnector1.CurrentDataRow!doc_origen) Or _
                '''   Not LibX.IsNull(LibXConnector1.CurrentDataRow!conc_serial) Then
                If e.Action = LibX.LibxConnectionActions.Edit Or _
                   Not LibX.IsNull(LibXConnector1.CurrentDataRow!conc_serial) Then
                ...
                '''End If
                ''''If Not LibX.IsNull(LibXConnector1.CurrentDataRow!tran_type) And _
                ''''      LibXConnector1.CurrentDataRow!tran_type = "SOL" Then
                ''''    Me.xtxt_amount.Enabled = True
                ''''    Me.xtxt_chequera.Enabled = True
                ''''    Me.xtxt_type_code.Enabled = False
                ''''    Me.xtxt_doc_numer.Enabled = False
    ...
    Private Sub LibXConnector1_ExecutedAction(ByVal sender As Object, ByVal e As LibX.ExecutingActionEventArgs) Handles LibXConnector1.ExecutedAction
        ...
        Try
            ...
            Or e.AcceptedAction = LibX.LibxConnectionActions.Edit Then
                ...
                'If oDocumento.EmitirDocumento(ParametrosEmision) > 0 Then
                '    MessageBox.Show("Se genero un cargo por emision de este documento!", "Cargo por Emision", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                'End If
    ...
    Private Sub LibXGrid1_CurrentRowChanged(ByVal sender As Object, ByVal e As LibX.LibXGrid.XDataGridCurrentRowChangedEventArgs) Handles LibXGrid1.CurrentRowChanged
        Try
            ...
            ''''    If LibXConnector1.IsDataEditing = True Then
            ''''        If LibXGrid1.GetValue(0, dgc_acct_no) = "" And e.row <> 0 Then
            ''''            dgc_acct_no.ReadOnly = True
            ''''            dgc_acct_no.isReadOnly = True
            ''''            Me.xlk_acct_no.Enabled = True
            ''''        Else
            ''''            dgc_acct_no.ReadOnly = False
            ''''            dgc_acct_no.isReadOnly = False
            ''''            Me.xlk_acct_no.Enabled = False
            ''''            If e.row = 0 And e.column = 0 Then
            ''''                LibXGrid1.Focus(1, 0)
            ''''            End If
            ''''        End If
            ''''    End If
    ...
    Private Sub xdt_doc_date_CurrentValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles xdt_doc_date.CurrentValueChanged
        ...
        End If
    End Sub
    Private Sub xdt_doc_date_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles xdt_doc_date.ValueChanged
        Try
            If LibXConnector1.IsDataEditing = True Then
                If xdt_doc_date.Value > LibX.Data.Manager.GetScalar("select getdate()") Then
                    xdt_doc_date.Value = LibX.Data.Manager.GetScalar("select getdate()")
