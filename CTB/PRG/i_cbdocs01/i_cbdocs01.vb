...
#End Region
    ...
    Private Sub LibXGrid1_CellValidate(ByVal sender As Object, ByVal e As LibX.LibXGrid.LibXGridCellValidateEventArgs) Handles LibXGrid1.CellValidate
        Try
            If LibXConnector1.IsDataEditing = True Then
                ...
                If e.cell = Me.dgc_acct_no.Column AndAlso LibXGrid1.GetValue(e.row, dgc_acct_no).Trim = "" Then
                    LibXGrid1.Focus(e.row, 0)
                    LibXGrid1.EndEdit(dgc_acct_no, e.row, True)
                    If mLastKeyPressedInGrid = Keys.Enter Then
                        e.hasErrors = True
                        Exit Sub
                    End If
                End If
                ''If mLastKeyPressedInGrid = Keys.Delete Then
                ''    e.hasErrors = True
                ''    Exit Sub
                ''End If
                ...
                ''If LibXGrid1.GetValue(CurrentIndex, Me.dgc_acct_no) = "1120102" _
                ''    Or LibXGrid1.GetValue(CurrentIndex, Me.dgc_acct_no) = "1120101" _
                ''    Or LibXGrid1.GetValue(CurrentIndex, Me.dgc_acct_no) = "1120202" _
                ''    Or LibXGrid1.GetValue(CurrentIndex, Me.dgc_acct_no) = "1120201" _
                ''    Or LibXGrid1.GetValue(CurrentIndex, Me.dgc_acct_no) = "1120103" Then
    ...
    Private Sub LibXConnector1_InsertingRow(ByVal sender As Object, ByVal e As LibX.Data.AdpaterRowUpdatingEventArgs) Handles LibXConnector1.InsertingRow
        ...
        Try
          ...
          Or e.UpdatingArgs.StatementType = StatementType.Update Then
                ...
                ''Else
                ''    e.UpdatingArgs.Row!perdr_year = oPeriodo.year
                ''    e.UpdatingArgs.Row!perdr_num = oPeriodo.Thru
                ''End If
            End If
            ...
            If e.UpdatingArgs.StatementType = StatementType.Update Then
                UpdSolicitudCK(e.UpdatingArgs.Row!cbm_serial)
            End If
    ...
    Private Sub xtxt_amount_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles xtxt_amount.Validating
        ...
        Try
            ...
            End If
            oTable = LibX.Data.Manager.GetDataTable(" select * from cgtrntpm   where Chequera =  '" & Me.xtxt_chequera.Text & "'  and tran_type  = '" & Me.xtxt_type_code.Text & "' and conciliable = 1 ")
            ...
            If oTable.Rows.Count > 0 And oTable.Rows(0)!origen = -1 Then
                ...
                If Me.LibXGrid1.getCurrentGridView.Count <= 0 And Val(Me.xtxt_amount.Text) > 0 Then
                    ...
                    LibXGrid1.SetValue(0, xdb_depto, 1)
            ...
            If oTable.Rows.Count > 0 And oTable.Rows(0)!origen = 1 Then
                ...
                If Me.LibXGrid1.getCurrentGridView.Count <= 0 Then
                    ...
                    If Me.xtxt_type_code.Text = "DEP" Then

                        oCuentas = LibX.Data.Manager.GetDataTable("select * from cuentascodificacion where type_code = '" & Me.xtxt_type_code.Text & "'")
                        acct = oCuentas.Rows(0)!acct_no

                        cbdocsd.Rows.Add(cbdocsd.NewRow)
                        LibXGrid1.SetValue(1, dgc_acct_no, acct)
                        LibXGrid1.SetValue(1, xdb_depto, 1)
    ...
    Private Sub LibXConnector1_ExecutingAction(ByVal sender As Object, ByVal e As LibX.ExecutingActionEventArgs) Handles LibXConnector1.ExecutingAction
        ...
        Try
            ...
            If e.Action = LibX.LibxConnectionActions.Edit Then
                If Not LibX.IsNull(LibXConnector1.CurrentDataRow!cbm_impreso) AndAlso (LibXConnector1.CurrentDataRow!cbm_impreso = 1 Or LibXConnector1.CurrentDataRow!tran_type <> "CHQ") Then
                    ActivactionEdit()
                    SetFocusToGrid()
                Else
                    InactivactionEdit()
    ...
    Private Sub LibXGrid1_CurrentRowChanged(ByVal sender As Object, ByVal e As LibX.LibXGrid.XDataGridCurrentRowChangedEventArgs) Handles LibXGrid1.CurrentRowChanged
        Try
            ...
            ''If LibXGrid1.GetValue(LibXGrid1.currentRowNum, Me.dgc_acct_no) = "1120102" _
            ''     Or LibXGrid1.GetValue(LibXGrid1.currentRowNum, Me.dgc_acct_no) = "1120101" _
            ''     Or LibXGrid1.GetValue(LibXGrid1.currentRowNum, Me.dgc_acct_no) = "1120202" _
            ''     Or LibXGrid1.GetValue(LibXGrid1.currentRowNum, Me.dgc_acct_no) = "1120201" _
            ''     Or LibXGrid1.GetValue(LibXGrid1.currentRowNum, Me.dgc_acct_no) = "1120103" Then
            ''    Me.dgc_acct_no.ReadOnly = True
            ''    Me.xdb_depto.ReadOnly = True
            ''    Me.dgc_credit_amount.ReadOnly = True
            ''    Me.dgc_debit_amount.ReadOnly = True
            ...
            ''    Me.xlk_acct_no.ShowLookup = False
            ''Else
            ''If LibXGrid1.GetValue(LibXGrid1.CurrentRowIndex, Me.dgc_acct_no) = "" Then
            ''    Me.dgc_acct_no.ReadOnly = False
            ''    Me.xdb_depto.ReadOnly = True
            ''    Me.dgc_credit_amount.ReadOnly = False
            ''    Me.dgc_debit_amount.ReadOnly = False
    ...
    Private Function sCodificaC()
        ...
        Try
            ...
            If mItbisRetenido > 0 Then
                ...
                LibXGrid1.SetValue(cntCuenta, xdb_depto, 1)
    ...
    Private Function InactivactionEdit()
        ...
        Me.TextBox4.Enabled = True
    End Function
    Public Function UpdSolicitudCK(ByVal code As Integer) As Boolean
        Dim sRow As DataRow
        Dim oTable As DataTable
        ...
        Try
            xSelect = New LibX.Data.XSelecStmt("cbdocsm")
            '// buscar el documento a Cobrar
            xSelect!cbm_serial = code
            oTable = xSelect.GetTable
            sRow = oTable.Rows(0)

            xInsert = New LibX.Data.XInsertStmt("cgxaudith", "auth_code")
            xInsert.Fields("doc_serial") = sRow!cbm_serial
            xInsert.Fields("referencia") = oTable.Rows(0)!doc_number & "-(ESTADO = " & oTable.Rows(0)!doc_status & ")-(CHEQUERA = " & oTable.Rows(0)!chequera & ")-(TIPO = " & oTable.Rows(0)!tran_type & ")-(MONTO = " & oTable.Rows(0)!amount & "FECHA =" & oTable.Rows(0)!doc_date & ")"
            xInsert.Fields("numero_doc") = oTable.Rows(0)!doc_number & "-(ESTADO = " & xcbo_doc_status.SelectedValue & ")-(CHEQUERA = " & xtxt_chequera.Text & ")-(TIPO = " & xtxt_type_code.Text & " FECHA = " & xdt_doc_date.Value & ")"
            xInsert.Fields("modulo") = "CTB"
            ...
            If oTable.Rows(0)!tran_type = "CHQ" Then
                xInsert.Fields("accion") = "MODIFICACION DEL CHEQUE " & sRow!doc_number & "-"
            End If
            If oTable.Rows(0)!tran_type = "DPT" Then
                xInsert.Fields("accion") = "MODIFICACION DE LA TRASNFERENCIA " & sRow!doc_number & "-"
            End If
            If oTable.Rows(0)!tran_type = "DEP" Then
                xInsert.Fields("accion") = "MODIFICACION DEL DEPOSITO " & sRow!doc_number & "-"
            End If
            xInsert.Execute()
